...
public class PCarbon {
    private IVCarbon ivCarbon;
    private DecimalFormat df = new DecimalFormat("0.00");
    private SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy/M/d HH:mm:ss:SSS", Locale.getDefault());

    public PCarbon(IVCarbon ivCarbon) {
        this.ivCarbon = ivCarbon;
    }

    public void init() {
        ArrayList<ECarbon> eCarbons = MyApplication.getAppData().geteCarBonList().getList();
        int size = eCarbons.size();
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i == 0) {
                ...
                float[] d = new float[1];
                Location.distanceBetween(eCarbons.get(i - 1).getLatitude(), eCarbons.get(i - 1).getLongitude(), lat, lng, d);
                double dis = Double.parseDouble(df.format(d[0]));
                distances.add(dis);
                double total = dis + distancesAdd.get(i - 1);
                distancesAdd.add(total);
                try {
                    long old = dateFormat.parse(eCarbons.get(i - 1).getDateFull()).getTime();
                    long now = dateFormat.parse(eCarbons.get(i).getDateFull()).getTime();
                    long sec = now - old;
        ...
        try {
            Date dateS = dateFormat.parse(MyApplication.getAppData().geteCarBonList().getList().get(0).getDateFull());
            Date dateE = dateFormat.parse(MyApplication.getAppData().geteCarBonList().getList().get(size - 1).getDateFull());
